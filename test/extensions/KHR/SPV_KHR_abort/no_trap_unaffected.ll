...
; use llvm.trap. Normal terminators (OpReturn, OpReturnValue, OpBranch,
...
; Round-trip must be lossless for non-trap code
; RUN: llvm-spirv -r %t.spv -o %t.rev.bc
; RUN: llvm-dis < %t.rev.bc | FileCheck %s --check-prefix=CHECK-LLVM

; No AbortKHR anywhere — only non-trap functions
...
; Unreachable preserved (not suppressed when no trap precedes it)
; CHECK-SPIRV: Function
...
; CHECK-LLVM: define spir_func void @has_unreachable
; CHECK-LLVM: unreachable

; CHECK-LLVM: define spir_func i32 @has_branch
; CHECK-LLVM: br i1
; CHECK-LLVM: ret i32
; CHECK-LLVM: ret i32
...
define spir_func void @has_unreachable() {
entry:
  unreachable
}

define spir_func i32 @has_branch(i32 %x) {
entry:

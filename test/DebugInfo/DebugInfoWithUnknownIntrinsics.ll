...
; RUN: llvm-as %s -o %t.bc
; RUN: amd-llvm-spirv %t.bc -o %t.spv --spirv-ext=+all --spirv-allow-unknown-intrinsics --experimental-debuginfo-iterators=1
; RUN: amd-llvm-spirv -r %t.spv -o %t.bc --experimental-debuginfo-iterators=1
; RUN: llvm-dis %t.bc -o %t.ll
; RUN: FileCheck %s --input-file %t.ll  -check-prefix=LLVM
